...
async function updateById(req: Request, res: Response) {
  try {
    ...
    const task = await Task.findOne({_id: taskId, isDeleted: false}).select('-isDeleted');
    ...
    update.description ? task.description = update.description : task.description;

    if (update.status === 'done' && task.status !== 'archive') {
      res.status(400).json({error: 'task status can only be changed to archive when it is done'});
      return;
    }
